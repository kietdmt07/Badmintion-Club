                      ...
                      return `<div style="border:1px solid #E1BEE7; border-radius:8px; background:var(--card-bg); padding:10px;">
                         ...
                         <div style="display:flex; flex-direction:column; gap:8px;">
                            ${g.pairs.map(pId => {
                               ...
                               return `<div style="display:flex; justify-content:space-between; align-items:center; background:var(--card-bg); border:1px solid #CCC; padding:6px 8px; border-radius:4px; font-size:13px;">
               ...
               <div style="display:flex; gap:20px; flex-wrap:wrap; align-items:flex-start;">
                 ...
                 <div style="flex:1.5; min-width:320px; display:flex; flex-direction:column; gap:12px;">
                   ${gMatches.map(m => {
                     ...
                     return `<div style="border:1px solid ${m.status==='live'?'#D32F2F':'var(--card-border)'}; border-radius:8px; padding:10px; background:${m.status==='finished'?'#F5F5F5':(m.status==='live'?'#FFEBEE':'#FFF')}; position:relative;">
                        ...
                        ${rMatches.map(m => {
                           ...
                           return `<div style="border:${isFinal?'2px solid #FFD700':'1px solid #CCC'}; border-radius:10px; padding:15px; background:${isFinal?'#FFFDE7':(m.status==='finished'?'#F5F5F5':(m.status==='live'?'#E8EAF6':'#FFF'))}; box-shadow:${isFinal?'0 6px 15px rgba(255,215,0,0.3)':'0 2px 5px rgba(0,0,0,0.1)'}; position:relative; transform: ${isFinal?'scale(1.05)':'scale(1)'};">
                             ...
                             <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                                <span style="font-size:14px; font-weight:${isP1Win?'bold':'600'}; color:${isP1Win?'#2E7D32':'#333'}">${isP1Win?'🏆 ':''}${p1 ? escapeHtml(p1.name) : '<span style="color:var(--text-secondary);font-style:italic;">Đang chờ nhánh dưới...</span>'}</span>
                             ...
                             <div style="display:flex; justify-content:space-between; align-items:center;">
                                <span style="font-size:14px; font-weight:${isP2Win?'bold':'600'}; color:${isP2Win?'#2E7D32':'#333'}">${p2 ? escapeHtml(p2.name) : '<span style="color:var(--text-secondary);font-style:italic;">Đang chờ nhánh dưới...</span>'}${isP2Win?' 🏆':''}</span>

function renderTournament(){
          ...
          <div style="display:flex; flex-direction:column; gap:12px;">
            ...
            <div style="display:flex; gap:16px; flex-wrap:wrap;">
              <div style="flex:1; min-width:140px;">
                <label style="font-size:13px; font-weight:600; color:#4E342E; display:block; margin-bottom:4px;">Số lượng sân thi đấu</label>
    ...
    if (activeTour) {
       ...
       const tabsUI = el(`<div style="display:flex; border-bottom:2px solid #E0E0E0; margin-bottom:20px; position:sticky; top:0; background:#FAFAFA; z-index:10; overflow-x:auto;">
         ...
         ${activeTour.status === 'grouping' ? `<div class="bc-tab ${isGroupingTab ? 'active' : ''}" style="flex:1; text-align:center; padding:12px; font-weight:bold; cursor:pointer; color:${isGroupingTab?'#9C27B0':'#666'}; border-bottom:${isGroupingTab?'3px solid #9C27B0':'none'}; white-space:nowrap;" id="tt-grouping">🎲 Gắp Thăm</div>` : ''}
       ...
       // Active Tour Header
       wrap.appendChild(el(`<div class="bc-card" style="display:flex; justify-content:space-between; align-items:center; padding:15px; background:#FFF3E0; border:1px solid #FFB74D; margin-bottom:1rem; position:relative;">
          <div>
             <h3 style="font-size:20px; color:#E65100; font-family:'Oswald',sans-serif; margin-bottom:5px; display:flex; align-items:center; gap:8px;">
             ...
             <div style="font-size:12px; color:#F57C00; font-weight:600;">Tình trạng: ${activeTour.status.toUpperCase()} | Sân thi đấu: ${activeTour.courtsCount || 4} sân</div>
          </div>
              ...
              ${activeTour.registrations.map(r => {
                ...
                return `<div style="display:flex; justify-content:space-between; align-items:center; padding:8px 12px; background:#F8F9FA; border:1px solid #E9ECEF; border-radius:6px;">
                  <div>
                    ...
                    ${r.type === 'pair' && p2 ? `<span style="color:#888; font-size:12px; margin:0 6px;">+</span> <strong style="font-size:13px; color:#333;">${escapeHtml(memberDisplayName(p2))}</strong> ${p2.gender==='Nữ'?'👩':'👨'} ${canManage() ? `<button class="bc-btn small warning" id="split-reg-${r.id}" style="margin-left:8px; padding:2px 6px; font-size:10px;">✂️ Tách cặp</button>` : ''}` : `<span style="font-size:11px; color:#E65100; margin-left:8px; font-style:italic;">(Đang tìm partner 🤝)</span>`}
                  </div>
       ...
       if (isGroupingTab) {
          ...
          groupingContainer.appendChild(el(`<div class="bc-card" style="background:#F3E5F5; border:1px solid #CE93D8;">
             <h4 style="color:#7B1FA2; margin-bottom:5px;">🎲 BƯỚC 2: GẮP THĂM & ĐIỀU CHỈNH BẢNG ĐẤU</h4>
             <div style="font-size:13px; color:#4A148C;">BTC có thể thay đổi bảng đấu cho từng Đội trước khi chốt. Hệ thống đã gợi ý chia đều ngẫu nhiên bên dưới.</div>
          </div>`));
          ...
          activeTour.categories.forEach(cat => {
             ...
             const catCard = el(`<div class="bc-card" style="border-left:4px solid #9C27B0;">
                <h4 style="color:#7B1FA2; margin-bottom:15px; font-size:16px;">🏸 Hạng mục: ${cat}</h4>
                <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap:15px;">
                   ${catGroups.map(g => {
                      return `<div style="border:1px solid #E1BEE7; border-radius:8px; background:#FAFAFA; padding:10px;">
                         <div style="font-weight:bold; color:#8E24AA; margin-bottom:10px; border-bottom:1px solid #E1BEE7; padding-bottom:5px;">${g.name} (${g.pairs.length} Đội)</div>
                         <div style="display:flex; flex-direction:column; gap:8px;">
                            ${g.pairs.map(pId => {
                               const p = activeTour.pairs.find(x => x.id === pId);
                               return `<div style="display:flex; justify-content:space-between; align-items:center; background:#FFF; border:1px solid #CCC; padding:6px 8px; border-radius:4px; font-size:13px;">
                            ...
                            ${g.pairs.length === 0 ? '<div style="color:#999; font-size:12px; font-style:italic;">Bảng trống</div>' : ''}
                         </div>
                      </div>`;
                   }).join('')}
                </div>
          ...
          if (canManage()) {
             groupingContainer.appendChild(el(`<div style="text-align:center; margin-top:20px; padding-top:20px; border-top:1px dashed #9C27B0; display:flex; flex-direction:column; gap:10px; align-items:center;">
                <button class="bc-btn" id="tour-start-playing-btn" style="background:#7B1FA2; border-color:#7B1FA2; font-size:16px; padding:12px 25px; box-shadow:0 4px 10px rgba(123,31,162,0.3);">⚡ PHÁT SINH LỊCH THI ĐẤU VÀ BẮT ĐẦU!</button>
                <button class="bc-btn danger" id="tour-rollback-btn" style="background:transparent; color:#D32F2F; border:none; font-size:13px; text-decoration:underline;">↩️ Quay xe: Hủy gắp thăm, trở về bước Ghi danh</button>
             </div>`));
       ...
       if (isGroupTab) {
          ...
          (activeTour.groups || []).forEach(g => {
             ...
             const groupCard = el(`<div class="bc-card" style="border-left: 4px solid #FF8F00;">
               <h4 style="color:#FF6F00; font-size:16px; margin-bottom:10px;">🏆 ${escapeHtml(g.name)} (${g.cat})</h4>
                 ...
                 <div style="flex:1.5; min-width:320px; display:flex; flex-direction:column; gap:12px;">
                   ${gMatches.map(m => {
                     ...
                     if (m.startTime && m.endTime) {
                        ...
                        durationText = `<span style="font-size:10px; color:#888;">⏱️ ${mins} phút</span>`;
                     } else if (m.startTime && m.status === 'live') {
                        const mins = Math.floor((Date.now() - m.startTime) / 60000);
                        durationText = `<span style="font-size:10px; color:#D32F2F; animation: pulse 1s infinite;">⏱️ Đang đánh (${mins}p)</span>`;
                     }

                     const isP1Win = m.status === 'finished' && m.score1 > m.score2;
                     const isP2Win = m.status === 'finished' && m.score2 > m.score1;

                     return `<div style="border:1px solid ${m.status==='live'?'#D32F2F':'#E0E0E0'}; border-radius:8px; padding:10px; background:${m.status==='finished'?'#F5F5F5':(m.status==='live'?'#FFEBEE':'#FFF')}; position:relative;">
                       ...
                       <div style="display:flex; justify-content:space-between; align-items:center;">
                         <div style="flex:1; font-size:13px; font-weight:${isP1Win ? 'bold':'600'}; color:${isP1Win?'#2E7D32':'#333'}">
                         ...
                         <div style="flex:1; font-size:13px; text-align:right; font-weight:${isP2Win ? 'bold':'600'}; color:${isP2Win?'#2E7D32':'#333'}">
       ...
       if (isKnockoutTab) {
          ...
          Object.keys(activeTour.bracket || {}).forEach(cat => {
             ...
             const catCard = el(`<div class="bc-card" style="border: 2px solid #3F51B5; border-radius:12px; overflow:hidden; padding:0;">
                ...
                <div style="padding:20px; display:flex; justify-content:space-around; align-items:flex-start; flex-wrap:wrap; gap:30px; background:linear-gradient(to right, #FAFAFA, #FFF); overflow-x:auto;">
                      ...
                      return `<div style="display:flex; flex-direction:column; gap:40px; flex:1; min-width:280px; position:relative;">
                        ${rMatches.map(m => {
                           ...
                           if (m.winnerTo) {
                              ...
                              if (nm) nextMatchInfo = `<div style="font-size:11px; color:#9C27B0; margin-top:5px; text-align:center; font-style:italic;">👉 Thắng vào: ${nm.name}</div>`;
                           }
                           ...
                           return `<div style="border:${isFinal?'2px solid #FFD700':'1px solid #CCC'}; border-radius:10px; padding:15px; background:${isFinal?'#FFFDE7':(m.status==='finished'?'#F5F5F5':(m.status==='live'?'#E8EAF6':'#FFF'))}; box-shadow:${isFinal?'0 6px 15px rgba(255,215,0,0.3)':'0 2px 5px rgba(0,0,0,0.1)'}; position:relative; transform: ${isFinal?'scale(1.05)':'scale(1)'};">
                             <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; border-bottom:1px dashed ${isFinal?'#FBC02D':'#EEE'}; padding-bottom:6px;">
                             ...
                             <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                                <span style="font-size:14px; font-weight:${isP1Win?'bold':'600'}; color:${isP1Win?'#2E7D32':'#333'}">${isP1Win?'🏆 ':''}${p1 ? escapeHtml(p1.name) : '<span style="color:#999;font-style:italic;">Đang chờ nhánh dưới...</span>'}</span>
                                ${canManage() && p1 && p2 ? `<input type="number" id="k-s1-${m.id}" value="${m.score1!==null?m.score1:''}" style="width:45px;text-align:center;border:1px solid ${isFinal?'#FBC02D':'#999'};border-radius:4px;padding:4px; font-weight:bold;"/>` : `<strong style="font-size:18px; color:${isFinal?'#D84315':'#000'};">${m.score1!==null?m.score1:'-'}</strong>`}
                             </div>
                             <div style="display:flex; justify-content:space-between; align-items:center;">
                                <span style="font-size:14px; font-weight:${isP2Win?'bold':'600'}; color:${isP2Win?'#2E7D32':'#333'}">${p2 ? escapeHtml(p2.name) : '<span style="color:#999;font-style:italic;">Đang chờ nhánh dưới...</span>'}${isP2Win?' 🏆':''}</span>
                                ${canManage() && p1 && p2 ? `<input type="number" id="k-s2-${m.id}" value="${m.score2!==null?m.score2:''}" style="width:45px;text-align:center;border:1px solid ${isFinal?'#FBC02D':'#999'};border-radius:4px;padding:4px; font-weight:bold;"/>` : `<strong style="font-size:18px; color:${isFinal?'#D84315':'#000'};">${m.score2!==null?m.score2:'-'}</strong>`}
                             </div>
                             ...
                             <div style="text-align:center; margin-top:12px; display:flex; gap:6px; justify-content:center;">
                               ...
                               <button class="bc-btn small" id="k-update-${m.id}" style="background:#3F51B5; border-color:#3F51B5; color:#FFF; font-size:11px; padding:4px 10px;">Lưu Tỷ số</button>
                             </div>` : ''}

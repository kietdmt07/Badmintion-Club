function renderTournament(){
            ...
            <div style="display:flex; gap:16px; flex-wrap:wrap;">
              <div style="flex:1; min-width:140px;">
                ...
                <select class="bc-select" id="ts-courts-count">
                  ...
                  <option value="5" ${state.tourSetup.courtsCount===5?'selected':''}>5 Sân</option>
                  <option value="6" ${state.tourSetup.courtsCount===6?'selected':''}>6 Sân</option>
                </select>
              </div>
    ...
    if (activeTour) {
       ...
       if (isGroupingTab) {
          ...
          if (canManage()) {
             groupingContainer.appendChild(el(`<div style="text-align:center; margin-top:20px; padding-top:20px; border-top:1px dashed #9C27B0;">
       ...
       if (isGroupTab) {
          ...
          const courtOptions = activeTour.selectedCourts.map(cId => {
             const c = state.courts.find(x => x.id === cId);
             return c ? `<option value="${c.id}">${escapeHtml(c.name)}</option>` : '';
          }).join('');
                   ...
                   ${gMatches.map(m => {
                     ...
                     return `<div style="border:1px solid ${m.status==='live'?'#D32F2F':'#E0E0E0'}; border-radius:8px; padding:10px; background:${m.status==='finished'?'#F5F5F5':(m.status==='live'?'#FFEBEE':'#FFF')}; position:relative;">
                       <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px; border-bottom:1px dashed #CCC; padding-bottom:4px;">
                          ...
                          ` : `<span style="font-size:11px; font-weight:bold; color:#F57F17; background:#FFFDE7; padding:2px 6px; border-radius:4px;">${m.courtId ? escapeHtml(state.courts.find(c=>c.id===m.courtId)?.name||'') : 'Chưa xếp sân'}</span>`}
                          ${durationText}
                       </div>
                        ...
                        ${rMatches.map(m => {
                           ...
                           return `<div style="border:${isFinal?'2px solid #FFD700':'1px solid #CCC'}; border-radius:10px; padding:15px; background:${isFinal?'#FFFDE7':(m.status==='finished'?'#F5F5F5':(m.status==='live'?'#E8EAF6':'#FFF'))}; box-shadow:${isFinal?'0 6px 15px rgba(255,215,0,0.3)':'0 2px 5px rgba(0,0,0,0.1)'}; position:relative; transform: ${isFinal?'scale(1.05)':'scale(1)'};">
                             <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; border-bottom:1px dashed ${isFinal?'#FBC02D':'#EEE'}; padding-bottom:6px;">
                                ...
                                ` : `<span style="font-size:10px; font-weight:bold; color:#1976D2; background:#E3F2FD; padding:2px 6px; border-radius:4px;">${m.courtId ? escapeHtml(state.courts.find(c=>c.id===m.courtId)?.name||'') : 'Chưa xếp sân'}</span>`}
                                ${durationText}
                             </div>
